...
def pca(X, K):
    ...
    #             
    ### TODO: YOUR CODE HERE
    D = cov(newData, rowvar = 0)
    eigVals, eigVects = linalg.eig(mat(D))
    eigValsOrder = argsort(eigVals);
    K_eiValsOder = eigValsOrder[-1: -(K + 1): -1]
    egvects = eigVects[:,K_eiValsOder]
    evals = real(eigVals[K_eiValsOder])
    evals = around(evals, 8)
    Z = real(egvects)
    P = real(newData*egvects)
    return (P, Z, evals)
