...
class SoftmaxRegression:
    ...
    def cost(self, X, Y, W=None):
        ...
        ### YOUR CODE HERE ###
        exp_shrunk_prediction = np.exp(W_X) 
        probabilities = (exp_shrunk_prediction / np.sum(exp_shrunk_prediction, axis=0))
        log_term = np.log(probabilities)
        cost = -1 * np.sum(np.multiply(indicator, log_term)) / N
        gradient = np.array(np.dot(X, (indicator - probabilities).T).T* (-1.0/N))
    ...
    def predict(self, X):
        ...
        #predicted_classes = np.argmax(W_X, axis=0)
